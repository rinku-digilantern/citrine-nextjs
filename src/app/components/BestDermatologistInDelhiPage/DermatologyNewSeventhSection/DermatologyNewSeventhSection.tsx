...
import styles from "./DermatologyNewSeventhSection.module.css";
...
const DermatologyNewSeventhSection: React.FC<Props> = ({ section }) => {
  if (!section) return null;
  return (
    <section className={styles.DermatologyNewSeventhSection}>
                ...
                <tr>
                  {section.table && section.table.length > 0 &&
                    Object.keys(section.table[0]).map((key) => (
                      <th key={key}><strong>{key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())}</strong></th>
                    ))}
                </tr>
...
export default DermatologyNewSeventhSection;
